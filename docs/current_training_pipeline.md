...
- Ví dụ `en_train_size: 20000` nghĩa là lấy tối đa 20.000 mẫu train tiếng Anh sau khi lọc sạch.
...
1. Train tiếng Việt trước bằng profile `train_vi`.
2. Lấy `outputs/checkpoints_vi/best_model` làm checkpoint khởi tạo.
3. Train tiếp tiếng Anh bằng profile `train_en_from_vi_eval_vi`.
4. Vẫn eval trên validation tiếng Việt trong bước 2.
...
```bash
.venv/bin/python src/model/training.py --profiles train_vi train_en_from_vi_eval_vi
```
...
evaluate QA metrics nếu track_eval_metrics=true
    -> EM / accuracy / precision / recall / F1
...
outputs/checkpoints_en_from_vi_eval_vi/
...
Eval checkpoint tiếng Việt trên validation tiếng Việt:

```bash
.venv/bin/python src/model/evalmodel.py \
  --checkpoint outputs/checkpoints_vi/best_model \
  --profile eval_vi \
  --save_dir outputs/eval_train_vi_on_vi
```

Eval checkpoint train tiếng Anh từ checkpoint Việt, vẫn trên validation tiếng Việt:

```bash
.venv/bin/python src/model/evalmodel.py \
  --checkpoint outputs/checkpoints_en_from_vi_eval_vi/best_model \
  --profile eval_vi \
  --save_dir outputs/eval_en_from_vi_eval_vi_on_vi
```
